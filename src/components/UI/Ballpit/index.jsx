...
// Função utilitária para calcular count responsivo
const getResponsiveCount = (baseCount) => {
  ...
  if (width < 640) {
    // Mobile
    return Math.floor(baseCount * 0.15) // 40% do count original
  } else if (width < 1024) {
    // Tablet
    return Math.floor(baseCount * 0.7) // 70% do count original
  } else {
    // Desktop
    return baseCount // 100% do count original
  }
}
...
const Ballpit = ({
  ...
  const [responsiveCount, setResponsiveCount] = useState(() =>
    getResponsiveCount(count)
  )

  useEffect(() => {
    const updateCount = () => {
      const newCount = getResponsiveCount(count)
      setResponsiveCount(newCount)

      // Se já existe uma instância, atualiza o count
      if (spheresInstanceRef.current) {
        spheresInstanceRef.current.setCount(newCount)
      }
    }

    // Define o count inicial
    updateCount()
  ...
  }, [responsiveCount, followCursor, props])
